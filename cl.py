...
BLACKLIST_FILE = "/home/keith/cl_blacklist.txt" # accumulates the results from searches previously run
...
if len(sys.argv) > 1:
    ...
    try:    # if no command line parameters, look for SEARCHES_FILE which should have one search per line
        ...
        searches = list(filter(None, searches))   # in case there's an empty string left over from a trailing \n in file
        f.close()
    except:
